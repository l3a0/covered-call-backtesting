...
- **Blog figure embeds.** Every blog post embeds PNGs by *relative* path `../docs/figures/NN_*.png` (resolves from `blog/` to repo-root `docs/figures/`; Substack re-uploads on publish, so the path only matters for in-repo/GitHub rendering). Each embed carries descriptive alt text and an italic caption, and the captions restate pinned numbers in plain language. The PNGs are regenerated by `make_figures.py` (`fig1`–`fig12`). When a figure's underlying data or labels change — a re-pin, an IV/regime retune, refreshed MSFT data — three things drift together and must move as one: the PNG, its alt text, and its caption. Current map: **Post 1** → `01_equity_curves`, `12_premium_waterfall`; **Post 2** → `06_delta_dial`, `05_implied_vs_realized_vol`; **Post 3** → `07_walk_forward_schematic`, `08_is_vs_oos`, `09_monte_carlo`, `10_regime_pnl`; **Post 4** → `01_equity_curves` (reused as the "reveal"), `03_bias_variance`, `11_excess_acf`, `02_excess_histogram`, `04_t_stat_vs_years`. Post 1 and Post 4 deliberately share `01_equity_curves` — keep them the same image. New caption-pinned values beyond the post-4 list above: the Monte Carlo trio (`~657%` shuffle mean, `~870%` best shuffle, `~915%` real path) narrated in Post 3's `09_monte_carlo` caption/alt-text, and the regime per-day bars (`~$23` / `~$300` / `~$400`) in Post 3's `10_regime_pnl`.
...
- **Generated notebook** (`covered_call_backtest.ipynb`), built from the tutorial markdown + figure script by `make_notebook.py`. It is not hand-edited and not a sweep target — it's a *regeneration obligation*. Any change to `tutorial_covered_call_backtest.md` (or `make_figures.py`) leaves the notebook stale until regenerated. Note the notebook only embeds the *tutorial's* figures (`fig1_equity_curves`–`fig4_t_stat_vs_years`); the blog-only figures (`fig5`–`fig12`) are not embedded in it. So a change that adds or edits only `fig5`–`fig12` produces a **no-op `make_notebook.py` diff** — that empty diff is expected and correct, not a sign the regen was skipped. Still run `make_notebook.py` to confirm the no-op rather than assuming it.
...
# Every blog figure embed — confirm the PNG it points at actually exists
for f in blog/*.md; do
