...
The production implementation in [`cc_backtest.py::walk_forward_optimization`](https://github.com/l3a0/covered-call-backtesting/blob/main/cc_backtest.py#L887) is heavily commented — it carries the teaching content (window arithmetic diagram, boolean-indexing explainer, Sharpe-built-inside-out walkthrough, Bessel's correction, √252 annualization derivation, "rules are LOCKED — no re-tuning" emphasis) right next to the code that does the work. The fixing test [`test_cc_backtest.py::TestMsftTenYearRegression::test_walk_forward_optimization`](https://github.com/l3a0/covered-call-backtesting/blob/main/test_cc_backtest.py#L1392) pins the 15 walk-forward periods, the most-chosen parameters, and the cumulative OOS compound return on the bundled MSFT data.
...
Before trusting any of those numbers, ask a prior question: does a single 2-year training window even contain enough data to fit three parameters? Robert Pardo's *The Evaluation and Optimization of Trading Strategies* (2008) gives two ways to check, and they disagree here — which is the whole lesson.

The first is **degrees of freedom** in the literal sense. Each free parameter you optimize consumes one degree of freedom, and each indicator consumes data equal to its lookback — the 30-day rolling-volatility window "uses up" its first 30 bars. So a 504-day window (2 years × 252 trading days) spends `3 + 30 = 33` and keeps `471` free, or **93.5%**. Pardo's rule of thumb wants that above ~90% (the [quantstrat](https://rdrr.io/github/braverock/quantstrat/man/degrees.of.freedom.html) port of his method tightens it to 95%); 93.5% clears the looser bar. By this measure the optimization is fine. [`degrees_of_freedom`](https://github.com/l3a0/covered-call-backtesting/blob/main/cc_backtest.py#L1096) computes it, and `TestDegreesOfFreedom` pins the arithmetic.

The second check is the one that bites. Pardo argues — in separate chapters on sample size and "How Many Trades?" — that the unit of statistical evidence is the **trade**, not the bar. And here the bar count lies. A covered call holds one short option for weeks, so consecutive daily bars are not independent observations: a single position drives P&L for its entire life. The 504 bars are really only as many independent events as there are option cycles. Across the 27-combo grid, a 2-year window generates a median of just **24 trades** (range 12–50), and even the *winning* configuration clears the conventional 30-trade floor in only 8 of the 15 walk-forward windows. By the measure that matters, the data is thin.

![Two panels. Left: a single horizontal bar representing 504 in-sample trading days, almost entirely green ("free," 471 bars, 93%) with a thin orange slice ("consumed," 3 parameters plus a 30-bar lookback) and a dashed red line at the 90% Pardo floor — the bar clears it easily. Right: fifteen vertical bars, one per walk-forward window, showing the winning fit's in-sample trade count; seven are red (below the dashed 30-trade floor), eight green, and the median sits exactly on the line.](docs/figures/13_degrees_of_freedom.png)

*The same fit, two verdicts. The bar-level degrees-of-freedom budget (left) passes Pardo's 90% floor with room to spare. The trade count (right) — the unit that actually carries statistical evidence for a held-position strategy — strains it: 7 of 15 windows fall short of 30, and the median sits right on the floor. Bar-counting flatters an overlay because one option drives many correlated days of P&L.*

So the degrees-of-freedom check **passes on paper and strains in substance**, and the gap between the two is the point. The bar-level 93.5% is necessary, not sufficient: it confirms the model isn't grossly over-parameterized, but it cannot see that a held-position overlay's daily returns are autocorrelated, so it overstates the real evidence. The binding constraints are the thin trade count and — the sharper test Part 5 builds — the Newey-West significance of the overlay's *excess* return over buy-and-hold. A clean degrees-of-freedom percentage never means the edge is real; it only means there was room to look for one.
...
The implementation is [`cc_backtest.py::monte_carlo_shuffle`](https://github.com/l3a0/covered-call-backtesting/blob/main/cc_backtest.py#L1183) — it computes daily returns from the real prices, shuffles their order with a fixed seed, rebuilds a synthetic price path from each shuffled sequence, runs the overlay backtest on each, then compares the real ordered path's total return against the distribution. The fixing test [`test_cc_backtest.py::TestMsftTenYearRegression::test_monte_carlo_shuffle`](https://github.com/l3a0/covered-call-backtesting/blob/main/test_cc_backtest.py#L1288) pins the resulting percentile, MC mean, and best-shuffled return on the bundled MSFT data (500 paths, `seed=42`, `__main__` params).
...
The implementation is [`cc_backtest.py::sensitivity_analysis`](https://github.com/l3a0/covered-call-backtesting/blob/main/cc_backtest.py#L1264) — it sweeps `call_delta` and `close_at_pct` at ±0.05 / ±0.10 / ±0.20 offsets from base. The algorithm: hold all params fixed except one, vary that one by a small offset in both directions, measure each variant's total return, then compute the worst drop from base. The fixing test [`test_cc_backtest.py::TestMsftTenYearRegression::test_sensitivity_perturbations`](https://github.com/l3a0/covered-call-backtesting/blob/main/test_cc_backtest.py#L1246) pins each variant's return and asserts the worst drop from base stays under 10% (the "robust" verdict) on the bundled MSFT data — the notebook companion calls the same function, so the two can't drift.
...
| **Degrees of freedom** (Part 4) | Too few data points per parameter | Pardo's precondition check: `observations − free parameters − indicator lookback`, kept above ~90% (bar-level), *and* the ~30-trade sample-size floor. Here the bar-level passes (~93.5%) but the trade count strains (median ~24) — a reminder that for a held-position overlay the bar count flatters the fit, so this is necessary, not sufficient. |
...
- **Degrees of freedom (Pardo):** A measure of how much room a dataset gives you to fit a strategy's parameters. Robert Pardo's bar-level version subtracts the free parameters and each indicator's lookback from the total observations and asks that ~90% remain (a 2-year, 504-bar window spends `3 + 30 = 33` and keeps 93.5%). But for a held-position strategy the bar count is misleading — one option drives many correlated days of P&L — so Pardo *also* checks the **number of trades** (the conventional floor is ~30), which is the unit that actually carries statistical evidence. Here the bar-level check passes and the trade count (median ~24) does not: a clean percentage is necessary, not sufficient. See `degrees_of_freedom` in the code.
...
- Pardo, R. (2008). *The Evaluation and Optimization of Trading Strategies* (2nd ed.). John Wiley & Sons. Source of walk-forward analysis; the *walk-forward efficiency* (out-of-sample ÷ in-sample) heuristic, whose "roughly two-thirds" band is practitioner lore, not a precise threshold from the book; and the *degrees-of-freedom* check used in Part 4 — both the bar-level "% remaining" formula (`observations − parameters − indicator lookback`, kept above ~90%) and the separate trade-count / "How Many Trades?" sample-size floor (~30). The bar-level 90% threshold is likewise a rule of thumb, not a law. ([Wiley](https://www.wiley.com/en-us/The+Evaluation+and+Optimization+of+Trading+Strategies%2C+2nd+Edition-p-9781118045053)) The bar-level formula is operationalized in code by the [quantstrat `degrees.of.freedom`](https://rdrr.io/github/braverock/quantstrat/man/degrees.of.freedom.html) function, which tightens the threshold to 95%.
