...
![A schedule of fifteen stacked rows, one per walk-forward cycle, time on the horizontal axis from 2016 to 2026. Each row shows a two-year training bar followed immediately by a six-month test bar; successive rows shift six months later, so the windows march diagonally down and to the right.](../docs/figures/07_walk_forward_schematic.png)

*Fifteen rolling exams. Each blue bar is two years the search was free to optimize on; the orange bar right after it is six months of fresh data where the rules were frozen. Only the orange stretches count toward the honest score.*
...
![Three cumulative-return curves over the 2018–2025 walk-forward span. The optimized fixed-defaults curve ends highest at +563%, the honest out-of-sample curve at +483%, and buy-and-hold Microsoft just below at +467%. All three track closely for most of the span and only fan apart late.](../docs/figures/08_is_vs_oos.png)
...
*Five hundred scrambles, and the real path's return sits to the right of all of them. If the strategy depended on the specific sequence of history — the trend that happened to follow that dip — destroying the order would have killed it. It didn't.*
...
The third is **regime analysis**. Bucket every trade's profit by the market state it closed in — bull, bear, or sideways — and check whether the strategy secretly depends on one of them. The covered-call overlay is profitable in all three, and the breakdown is the most interesting result in the whole project: it earns roughly <span>$</span>23 of premium per day in bull markets and <span>$</span>300–<span>$</span>400 per day in bear and sideways ones. It's structurally defensive — it earns most of its keep precisely when the market isn't going straight up, which is the entire point of selling insurance.
