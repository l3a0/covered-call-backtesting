...
The second is that the clean version is priced for institutions. The datasets that do have rigorous historical option prices and implied vols, like OptionMetrics, are sold to funds and universities at fund-and-university prices. For an individual building an educational backtest, that's a wall. And it's the quiet reason so many retail backtests of options strategies don't disclose where their volatility came from: confronting the question costs money, so they don't confront it.
...
The multipliers run the way they do for a real reason. Implied volatility tends to sit 20–40% above realized volatility — option buyers pay up for insurance, the way drivers overpay for peace of mind. But that gap is widest exactly when markets are calm and complacent, and it compresses when volatility is already elevated and implied vol has nowhere left to climb. A flat markup would miss that; the regime dial approximates it.
